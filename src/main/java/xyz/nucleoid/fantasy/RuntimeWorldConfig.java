...
public final class RuntimeWorldConfig {
    ...
    public RuntimeWorldConfig setGameRule(GameRules.RuleKey<GameRules.BooleanValue> key, boolean value) {
        this.gameRules.set(key, value);
        return this;
    }

    public RuntimeWorldConfig setGameRule(GameRules.RuleKey<GameRules.IntegerValue> key, int value) {
        this.gameRules.set(key, value);
        return this;
    }
    ...
    public Dimension createDimensionOptions(MinecraftServer server) {
        DimensionType dimensionType = this.resolveDimensionType(server);
        return new Dimension(() -> dimensionType, this.generator);
    }

    private DimensionType resolveDimensionType(MinecraftServer server) {
        DimensionType dimensionType = this.dimensionType;
        if (dimensionType == null) {
            dimensionType = server.registryAccess().dimensionTypes().get(this.dimensionTypeKey);
