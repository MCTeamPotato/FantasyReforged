 ...
 * It supports both temporary dimensions which do not get saved, and persistent dimensions which can be safely
...
public final class Fantasy {
    ...
    public static final RegistryKey<DimensionType> DEFAULT_DIM_TYPE = RegistryKey.create(Registry.DIMENSION_TYPE_REGISTRY, new ResourceLocation(Fantasy.ID, "default"));
    ...
    private RuntimeWorld addTemporaryWorld(RuntimeWorldConfig config) {
        RegistryKey<World> worldKey = RegistryKey.create(Registry.DIMENSION_REGISTRY, generateTemporaryWorldKey());

        try {
            SaveFormat.LevelSave session = this.serverAccess.getSession();
            FileUtils.forceDeleteOnExit(session.getDimensionPath(worldKey));
    ...
    private List<RuntimeWorld> collectTemporaryWorlds() {
        ...
        for (ServerWorld world : this.server.getAllLevels()) {
